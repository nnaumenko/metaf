...
///////////////////////////////////////////////////////////////////////////////
// Non-variable runway visual range
// Purpose: to confirm that non-variable runway visual range groups are parsed 
// correctly
///////////////////////////////////////////////////////////////////////////////
...
TEST(RunwayVisualRangeGroup, parseSingleValueMeters) {
	...
	ASSERT_TRUE(rvrg->visualRange().distance().has_value());
	EXPECT_NEAR(rvrg->visualRange().distance().value(), 1100, margin);
...
TEST(RunwayVisualRangeGroup, parseRunwayWithDesignator) {
	...
	ASSERT_TRUE(rvrg1->visualRange().distance().has_value());
	EXPECT_NEAR(rvrg1->visualRange().distance().value(), 1100, margin);
	...
	ASSERT_TRUE(rvrg2->visualRange().distance().has_value());
	EXPECT_NEAR(rvrg2->visualRange().distance().value(), 1100, margin);
	...
	ASSERT_TRUE(rvrg3->visualRange().distance().has_value());
	EXPECT_NEAR(rvrg3->visualRange().distance().value(), 1100, margin);
...
TEST(RunwayVisualRangeGroup, parseSingleValueMetersWithModifiers) {
	...
	EXPECT_TRUE(rvrg1->visualRange().isReported());
	ASSERT_TRUE(rvrg1->visualRange().distance().has_value());
	EXPECT_NEAR(rvrg1->visualRange().distance().value(), 2000, margin);
	EXPECT_EQ(rvrg1->visualRange().unit(), metaf::Distance::Unit::METERS);
	EXPECT_FALSE(rvrg1->isVariableVisualRange());
	EXPECT_FALSE(rvrg1->minVisualRange().isReported());
	EXPECT_FALSE(rvrg1->maxVisualRange().isReported());
	...
	EXPECT_TRUE(rvrg2->visualRange().isReported());
	ASSERT_TRUE(rvrg2->visualRange().distance().has_value());
	EXPECT_NEAR(rvrg2->visualRange().distance().value(), 200, margin);
	EXPECT_EQ(rvrg2->visualRange().unit(), metaf::Distance::Unit::METERS);
	EXPECT_FALSE(rvrg2->isVariableVisualRange());
	EXPECT_FALSE(rvrg2->minVisualRange().isReported());
	EXPECT_FALSE(rvrg2->maxVisualRange().isReported());
...
TEST(RunwayVisualRangeGroup, parseSingleValueFeet) {
	...
	EXPECT_TRUE(rvrg->visualRange().isReported());
	ASSERT_TRUE(rvrg->visualRange().distance().has_value());
	EXPECT_NEAR(rvrg->visualRange().distance().value(), 4500, margin);
...
TEST(RunwayVisualRangeGroup, parseSingleValueFeetWithModifiers) {
	...
	EXPECT_TRUE(rvrg1->visualRange().isReported());
	ASSERT_TRUE(rvrg1->visualRange().distance().has_value());
	EXPECT_NEAR(rvrg1->visualRange().distance().value(), 6000, margin);
	...
	EXPECT_TRUE(rvrg2->visualRange().isReported());
	ASSERT_TRUE(rvrg2->visualRange().distance().has_value());
	EXPECT_NEAR(rvrg2->visualRange().distance().value(), 400, margin);
...
TEST(RunwayVisualRangeGroup, parseSingleValueWithTrends) {
	...
	EXPECT_TRUE(rvrg2->visualRange().isReported());
	ASSERT_TRUE(rvrg2->visualRange().distance().has_value());
	EXPECT_NEAR(rvrg2->visualRange().distance().value(), 1000, margin);
	EXPECT_EQ(rvrg2->visualRange().unit(), metaf::Distance::Unit::FEET);
	EXPECT_FALSE(rvrg2->isVariableVisualRange());
	EXPECT_FALSE(rvrg2->minVisualRange().isReported());
	EXPECT_FALSE(rvrg2->maxVisualRange().isReported());
	...
	ASSERT_TRUE(rvrg4->visualRange().distance().has_value());
	EXPECT_NEAR(rvrg4->visualRange().distance().value(), 1000, margin);
...
///////////////////////////////////////////////////////////////////////////////
// Variable runway visual range
// Purpose: to confirm that variable runway visual range groups are parsed 
// correctly
///////////////////////////////////////////////////////////////////////////////
...
TEST(RunwayVisualRangeGroup, parseVariableValueMeters) {
	...
	ASSERT_TRUE(rvrg->minVisualRange().distance().has_value());
	EXPECT_NEAR(rvrg->minVisualRange().distance().value(), 500, margin);
	...
	ASSERT_TRUE(rvrg->maxVisualRange().distance().has_value());
	EXPECT_NEAR(rvrg->maxVisualRange().distance().value(), 800, margin);
...
TEST(RunwayVisualRangeGroup, parseVariableValueFeet) {
	...
	EXPECT_EQ(rvrg->minVisualRange().modifier(), metaf::Distance::Modifier::NONE);
	ASSERT_TRUE(rvrg->minVisualRange().distance().has_value());
	EXPECT_NEAR(rvrg->minVisualRange().distance().value(), 600, margin);
	EXPECT_EQ(rvrg->minVisualRange().unit(), metaf::Distance::Unit::FEET);

	EXPECT_TRUE(rvrg->maxVisualRange().isReported());
	EXPECT_EQ(rvrg->maxVisualRange().modifier(), metaf::Distance::Modifier::NONE);
	ASSERT_TRUE(rvrg->maxVisualRange().distance().has_value());
	EXPECT_NEAR(rvrg->maxVisualRange().distance().value(), 1200, margin);
...
TEST(RunwayVisualRangeGroup, parseVariableValueMetersWithModifiers) {
	...
	ASSERT_TRUE(rvrg1->minVisualRange().distance().has_value());
	EXPECT_NEAR(rvrg1->minVisualRange().distance().value(), 500, margin);
	...
	ASSERT_TRUE(rvrg1->maxVisualRange().distance().has_value());
	EXPECT_NEAR(rvrg1->maxVisualRange().distance().value(), 800, margin);
	...
	ASSERT_TRUE(rvrg2->minVisualRange().distance().has_value());
	EXPECT_NEAR(rvrg2->minVisualRange().distance().value(), 500, margin);
	...
	ASSERT_TRUE(rvrg2->maxVisualRange().distance().has_value());
	EXPECT_NEAR(rvrg2->maxVisualRange().distance().value(), 800, margin);
	...
	ASSERT_TRUE(rvrg3->minVisualRange().distance().has_value());
	EXPECT_NEAR(rvrg3->minVisualRange().distance().value(), 500, margin);
	...
	ASSERT_TRUE(rvrg3->maxVisualRange().distance().has_value());
	EXPECT_NEAR(rvrg3->maxVisualRange().distance().value(), 800, margin);
...
TEST(RunwayVisualRangeGroup, parseVariableValueFeetWithModifiers) {
	...
	EXPECT_EQ(rvrg1->minVisualRange().modifier(), metaf::Distance::Modifier::LESS_THAN);
	ASSERT_TRUE(rvrg1->minVisualRange().distance().has_value());
	EXPECT_NEAR(rvrg1->minVisualRange().distance().value(), 600, margin);
	EXPECT_EQ(rvrg1->minVisualRange().unit(), metaf::Distance::Unit::FEET);
	EXPECT_TRUE(rvrg1->maxVisualRange().isReported());
	EXPECT_EQ(rvrg1->maxVisualRange().modifier(), metaf::Distance::Modifier::NONE);
	ASSERT_TRUE(rvrg1->maxVisualRange().distance().has_value());
	EXPECT_NEAR(rvrg1->maxVisualRange().distance().value(), 1200, margin);
	...
	EXPECT_EQ(rvrg2->minVisualRange().modifier(), metaf::Distance::Modifier::NONE);
	ASSERT_TRUE(rvrg2->minVisualRange().distance().has_value());
	EXPECT_NEAR(rvrg2->minVisualRange().distance().value(), 600, margin);
	EXPECT_EQ(rvrg2->minVisualRange().unit(), metaf::Distance::Unit::FEET);
	EXPECT_TRUE(rvrg2->maxVisualRange().isReported());
	EXPECT_EQ(rvrg2->maxVisualRange().modifier(), metaf::Distance::Modifier::MORE_THAN);
	ASSERT_TRUE(rvrg2->maxVisualRange().distance().has_value());
	EXPECT_NEAR(rvrg2->maxVisualRange().distance().value(), 1200, margin);
	...
	EXPECT_EQ(rvrg3->minVisualRange().modifier(), metaf::Distance::Modifier::LESS_THAN);
	ASSERT_TRUE(rvrg3->minVisualRange().distance().has_value());
	EXPECT_NEAR(rvrg3->minVisualRange().distance().value(), 600, margin);
	EXPECT_EQ(rvrg3->minVisualRange().unit(), metaf::Distance::Unit::FEET);
	EXPECT_TRUE(rvrg3->maxVisualRange().isReported());
	EXPECT_EQ(rvrg3->maxVisualRange().modifier(), metaf::Distance::Modifier::MORE_THAN);
	ASSERT_TRUE(rvrg3->maxVisualRange().distance().has_value());
	EXPECT_NEAR(rvrg3->maxVisualRange().distance().value(), 1200, margin);
...
TEST(RunwayVisualRangeGroup, parseVariableValueWithTrends) {
	...
	ASSERT_TRUE(rvrg1->minVisualRange().distance().has_value());
	EXPECT_NEAR(rvrg1->minVisualRange().distance().value(), 250, margin);
	EXPECT_EQ(rvrg1->minVisualRange().unit(), metaf::Distance::Unit::METERS);
	EXPECT_TRUE(rvrg1->maxVisualRange().isReported());
	EXPECT_EQ(rvrg1->maxVisualRange().modifier(), metaf::Distance::Modifier::NONE);
	ASSERT_TRUE(rvrg1->maxVisualRange().distance().has_value());
	EXPECT_NEAR(rvrg1->maxVisualRange().distance().value(), 450, margin);
	EXPECT_EQ(rvrg1->maxVisualRange().unit(), metaf::Distance::Unit::METERS);
	...
	ASSERT_TRUE(rvrg2->minVisualRange().distance().has_value());
	EXPECT_NEAR(rvrg2->minVisualRange().distance().value(), 250, margin);
	EXPECT_EQ(rvrg2->minVisualRange().unit(), metaf::Distance::Unit::METERS);
	EXPECT_TRUE(rvrg2->maxVisualRange().isReported());
	EXPECT_EQ(rvrg2->maxVisualRange().modifier(), metaf::Distance::Modifier::MORE_THAN);
	ASSERT_TRUE(rvrg2->maxVisualRange().distance().has_value());
	EXPECT_NEAR(rvrg2->maxVisualRange().distance().value(), 450, margin);
	EXPECT_EQ(rvrg2->maxVisualRange().unit(), metaf::Distance::Unit::METERS);
	EXPECT_EQ(rvrg2->trend(), metaf::RunwayVisualRangeGroup::Trend::DOWNWARD);
	...
	ASSERT_TRUE(rvrg3->minVisualRange().distance().has_value());
	EXPECT_NEAR(rvrg3->minVisualRange().distance().value(), 2600, margin);
	EXPECT_EQ(rvrg3->minVisualRange().unit(), metaf::Distance::Unit::FEET);
	EXPECT_TRUE(rvrg3->maxVisualRange().isReported());
	EXPECT_EQ(rvrg3->maxVisualRange().modifier(), metaf::Distance::Modifier::NONE);
	ASSERT_TRUE(rvrg3->maxVisualRange().distance().has_value());
	EXPECT_NEAR(rvrg3->maxVisualRange().distance().value(), 3200, margin);
	EXPECT_EQ(rvrg3->maxVisualRange().unit(), metaf::Distance::Unit::FEET);
	...
	ASSERT_TRUE(rvrg4->minVisualRange().distance().has_value());
	EXPECT_NEAR(rvrg4->minVisualRange().distance().value(), 5000, margin);
	...
	ASSERT_TRUE(rvrg4->maxVisualRange().distance().has_value());
	EXPECT_NEAR(rvrg4->maxVisualRange().distance().value(), 6000, margin);
...
TEST(RunwayVisualRangeGroup, parseExtraTrendDelimiter) {
	const auto rvrg = metaf::RunwayVisualRangeGroup::parse("R08/6000FT/", metaf::ReportPart::METAR);
	ASSERT_TRUE(rvrg.has_value());
	EXPECT_EQ(rvrg->runway().number(), 8u);
	EXPECT_EQ(rvrg->runway().designator(), metaf::Runway::Designator::NONE);
	EXPECT_EQ(rvrg->visualRange().modifier(), metaf::Distance::Modifier::NONE);
	EXPECT_TRUE(rvrg->visualRange().isReported());
	ASSERT_TRUE(rvrg->visualRange().distance().has_value());
	EXPECT_NEAR(rvrg->visualRange().distance().value(), 6000, margin);
	EXPECT_EQ(rvrg->visualRange().unit(), metaf::Distance::Unit::FEET);
	EXPECT_FALSE(rvrg->isVariableVisualRange());
	EXPECT_FALSE(rvrg->minVisualRange().isReported());
	EXPECT_FALSE(rvrg->maxVisualRange().isReported());
...
TEST(RunwayVisualRangeGroup, parseMissingTrendDelimiter) {
	const auto rvrg = metaf::RunwayVisualRangeGroup::parse("R08/6000FTD", metaf::ReportPart::METAR);
	ASSERT_TRUE(rvrg.has_value());
	EXPECT_EQ(rvrg->runway().number(), 8u);
	EXPECT_EQ(rvrg->runway().designator(), metaf::Runway::Designator::NONE);
	EXPECT_TRUE(rvrg->visualRange().isReported());
	...
	EXPECT_FALSE(rvrg->minVisualRange().isReported());
	EXPECT_FALSE(rvrg->maxVisualRange().isReported());
	EXPECT_EQ(rvrg->trend(), metaf::RunwayVisualRangeGroup::Trend::DOWNWARD);
}
...
///////////////////////////////////////////////////////////////////////////////
// Appending tests
// Purpose: to confirm that runway visual range groups do not append
///////////////////////////////////////////////////////////////////////////////
